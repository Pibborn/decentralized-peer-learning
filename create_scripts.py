...
def run():
    ...
    environments = ['HalfCheetahBulletEnv-v0',
    ...
    runnning_time = {
        ...
        'HalfCheetahPyBulletEnv-v0': '24:00:00',
    ...
    learning_rate = {
        # 'exponential_decay': "\"lambda x: (np.exp(x) - 1) * 7.3e-4 + 7.3e-4\" " \
        #                      "\"lambda x: (np.exp(x) - 1) * 7.3e-4 + 7.3e-4\" " \
        #                      "\"lambda x: (np.exp(x) - 1) * 7.3e-4 + 7.3e-4\" " \
        #                      "\"lambda x: (np.exp(x) - 1) * 7.3e-4 + 7.3e-4\" "
        # 'zero_learning_rate': "\"lambda x: 7.3e-4\" " \
        #                       "\"lambda x: 7.3e-4\" " \
        #                       "\"lambda x: 7.3e-4\" " \
        #                       "\"lambda x: 0\" "
        'normal_learning_rate': "\"lambda x: 7.3e-4\" " \
                              "\"lambda x: 7.3e-4\" " \
                              "\"lambda x: 7.3e-4\" " \
                              "\"lambda x: 7.3e-4\" "
    ...
    sample_from_suggestions_list = [True,False]
    experiment_list = []
                                    ...
                                    for use_advantage in use_advantage_list:
                                        for sample_from_suggestions in sample_from_suggestions_list:
                                            if sample_from_suggestions == True:
                                                ...
                                                experiment_name = create_script(agent_type, env, learning_rate,
                                                                                learning_rate_key, mix, a_num,
                                                                                runnning_time, scripts, switch_ratio,
                                                                                net_arch, script_folder, n_timesteps,
                                                                                buffer_size, follow_steps,
                                                                                use_advantage, epsilon, temperature,
                                                                                sample_from_suggestions)
                                                experiment_list.append(experiment_name)
...
def create_script(agent_type, enviroments, learning_rate, learning_rate_key, mix_agents, number_agent, runnning_time,
                  scripts, switch_ratio, net_arch, script_folder, n_timesteps, buffer_size, follow_steps, use_advantage,
                  epsilon, temperature, sample_from_suggestions):
    experiment_name = f'{agent_type}_{number_agent}_{enviroments}_{net_arch}_{learning_rate_key}' \
                      f'_adv_{use_advantage}_sample_sug_{sample_from_suggestions}_eps_{epsilon}_T_{temperature}'
    ...
    with open(f"{script_folder}/{experiment_name}.sh", "w") as file1:
        ...
        write_python_default_parameter(agent_type, enviroments, file1, learning_rate, learning_rate_key, net_arch,
                                       number_agent, scripts, n_timesteps, buffer_size, follow_steps)
        if agent_type == 'peer':
...
def write_python_default_parameter(agent_type, enviroments, file1, learning_rate, learning_rate_key, net_arch,
                                   number_agent, scripts, n_timesteps, buffer_size, follow_steps):
    ...
    #file1.writelines(f"  --load_paths \"delete_me/2022-08-31_15.49.55__0/trained_model.zip\"   \\\n")
    file1.writelines(f"  --agents_to_store 1  \\\n")
